...
import Authenticated from '../../components/AuthenticatedComponent/Authenticated';
...
function AuthScreen({ navigation }) {
    ...
    return (
        <Center flex={1} px={4} bg={backgroundBox}>
            <View mb={10}>
                <Text color={textColor} fontSize={28}>
                    Welcome to
                </Text>
                <Text color={textColor} fontSize={28}>
                    <Text color="#57cc99" bold="bold">
                        Garden
                    </Text>{' '}
                    App
                </Text>
            ...
            <Box alignItems="center" width="100%">
                {user ? (
                    ...
                    <>
                        {errorMessage ? (
                            <Text padding={4}>{errorMessage}</Text>
                        ) : null}
                        <LoginRegisterForm navigation={navigation} />
                    </>
                )}
            </Box>
        </Center>
